...
class WebSocket:
    ...
    async def connect(self):
        ...
        while not self.connected and (
            not is_finite_retry or attempt < self._max_reconnect_attempts
        ):
            attempt += 1
            self._lavalink._logger.info(
                "[NODE-{}] Attempting to establish WebSocket "
                "connection ({}/{})...".format(
                    self._node.name, attempt, max_attempts_str
                )
            )
            ...
            try:
                ...
                if isinstance(ce, aiohttp.ClientConnectorError):
                    self._lavalink._logger.warning(
                        "[NODE-{}] Invalid response received; this may indicate that "
                    ...
                    if ce.status in (
                        ...
                        self._lavalink._logger.warning(
                            "[NODE-{}] Authentication failed while trying to "
                            "establish a connection to the node.".format(
                                self._node.name
                            )
                        )
                    ...
                    self._lavalink._logger.warning(
                        "[NODE-{}] The remote server returned code {}, "
                        "the expected code was 101. This usually "
                        "indicates that the remote server is a webserver "
                        "and not Lavalink. Check your ports, and try again.".format(
                            self._node.name, ce.status
                        )
                    )
        ...
        self._lavalink._logger.warning(
            "[NODE-{}] A WebSocket connection could not be established within 3 "
    ...
    async def _listen(self):
        ...
        async for msg in self._ws:
            self._lavalink._logger.debug(
                "[NODE-{}] Received WebSocket message: {}".format(
                    self._node.name, msg.data
                )
            )
            ...
            if msg.type == aiohttp.WSMsgType.TEXT:
                ...
                self._lavalink._logger.error(
                    "[NODE-{}] Exception in WebSocket! {}.".format(self._node.name, exc)
                )
                break
            elif msg.type in self._closers:
                self._lavalink._logger.debug(
                    "[NODE-{}] Received close frame with code {}.".format(
                        self._node.name, msg.data
                    )
                )
    ...
    async def _websocket_closed(self, code: int = None, reason: str = None):
        ...
        """
        self._lavalink._logger.debug(
            "[NODE-{}] WebSocket disconnected with the following: code={} "
    ...
    async def _handle_message(self, data: dict):
        ...
        if op == "stats":
            ...
            self._lavalink._logger.warning(
                "[NODE-{}] Received unknown op: {}".format(self._node.name, op)
            )
    ...
    async def _handle_event(self, data: dict):
        ...
        if not player:
            self._lavalink._logger.warning(
                "[NODE-{}] Received event for non-existent player! GuildId: {}".format(
                    self._node.name, data["guildId"]
                )
            )
            return
        ...
        #    event = TrackStartEvent(player, player.current)
            ...
            self._lavalink._logger.warning(
                "[NODE-{}] Unknown event received: {}".format(
                    self._node.name, event_type
                )
            )
            return
    ...
    async def _send(self, **data):
        ...
        if self.connected:
            self._lavalink._logger.debug(
                "[NODE-{}] Sending payload {}".format(self._node.name, str(data))
            )
            await self._ws.send_json(data)
        else:
            self._lavalink._logger.debug(
                "[NODE-{}] Send called before WebSocket ready!".format(self._node.name)
            )
